...
class MeetingRepo:
    ...
    def add_meeting(self, first_user_id: int, second_user_id: int):
        if first_user_id > second_user_id:
            first_user_id, second_user_id = second_user_id, first_user_id

        return self.conn.fetchval(
            """with row as (
                    INSERT INTO meetings (t_user_id, time_matching) VALUES ($1, now()) RETURNING id)
                INSERT INTO meetings (id, t_user_id, time_matching) (
                SELECT id, $2, now()
